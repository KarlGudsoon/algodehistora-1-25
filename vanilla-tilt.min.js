...
let destroyBox = document.querySelectorAll(".widget-personaje-flip");

destroyBox.forEach(function(box) {
    let tiltEnabled = false; // Estado individual del tilt para cada elemento

    box.addEventListener("click", function () {
        // Verificar si el tilt está activado para este elemento
        if (!tiltEnabled) {
            // Inicializar VanillaTilt en este elemento
            VanillaTilt.init(box);
            tiltEnabled = true; // Actualizar el estado a activado
        } else {
            // Destruir VanillaTilt en este elemento
            box.vanillaTilt.destroy();
            tiltEnabled = false; // Actualizar el estado a desactivado
        }
    });
});

let cartas = document.querySelector("#personaje-siderurgica");
VanillaTilt.init(cartas);

function checkScreenWidth() {
  if (window.innerWidth < 768) {
    console.log("La pantalla es menor que 768px");
    cartas.vanillaTilt.destroy();
  } else {
    console.log("La pantalla es mayor o igual a 768px");
    VanillaTilt.init(cartas);
  }
}
...
// Escuchar el evento resize para actualizar dinámicamente
