...
function paisClic() {
    var idpais = this.id;
    var pais = document.getElementById("card-" + idpais);
    if (pais) {
        var bottomPosition = pais.style.bottom;
        if (bottomPosition === "30px") {
            pais.style.bottom = "-500px"; // Cerrar el concepto
        } else {
            pais.style.bottom = "30px"; // Abrir el concepto
        }
    }
}
...
pais.forEach(function(pais) {
    pais.addEventListener("click", paisClic);
});
